...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("commutercoin"))
        return false;
...
{
    // Convert commutercoin:// to commutercoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("commutercoin://"))
    {
        uri.replace(0, 12, "commutercoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CommuterCoin.lnk";
}
...
{
    return GetAutostartDir() / "commutercoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("CommuterCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  commutercoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("CommuterCoin-Qt"));
